...
public class UnmuteCmd extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        {
            event.reply(event.getClient().getError()+" Up to 20 users can be unmuted at once.");
            return;
        }
        ...
        List<Member> members = event.getMessage().getMentionedUsers().stream()
                ...
                .filter((Member m) -> {
                    ...
                    {
                        builder.append("\n").append(event.getClient().getWarning()).append(" Member ").append(FormatUtil.formatUser(m.getUser())).append(" is not muted!");
                        return false;
                    }
                    return true;
        ...
        else for(int i=0; i<members.size(); i++)
        {
            ...
            event.getGuild().getController().removeRolesFromMember(m, muteRole).queue(v -> {
                    ...
                    builder.append("\n").append(event.getClient().getError()).append(" I failed to unmute ").append(FormatUtil.formatUser(m.getUser()));
                    if(last)
                        event.reply(builder.toString());
            });
        }
