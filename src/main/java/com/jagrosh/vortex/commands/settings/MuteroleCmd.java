 ...
 */
public class MuteroleCmd extends Command
{
    private final Vortex vortex;
    
    public MuteroleCmd(Vortex vortex)
    {
        this.vortex = vortex;
        this.name = "muterole";
        this.help = "sets the muted role";
        this.aliases = new String[]{"mutedrole"};
        this.arguments = "<role>";
    ...
    protected void execute(CommandEvent event)
    {
        ...
        {
            vortex.getDatabase().settings.setMutedRole(event.getGuild(), null);
            event.replySuccess("Vortex will use Timeouts instead of a Mute role.");
            return;
        }
        ...
        {
            vortex.getDatabase().settings.setMutedRole(event.getGuild(), roles.get(0));
            event.replySuccess("`"+roles.get(0).getName()+"` will now be applied when a user is Muted");
        }
        else
